...
export class AuthenticationController {
    ...
    async loginUser(@Body() userCredentials: UserCredentials): Promise<LoginResponse> {
        ...
        if (AUTH_USE_TEST_USERS) {
            ...
            return this.loginWithTestUser(userCredentials.username, userCredentials.password)
        } else {
            this.logger.log('using auth0 login strategy')

            return this.loginWithAuth0(userCredentials.username, userCredentials.password)
        }
    }

    async loginWithTestUser(username: string, password: string): Promise<LoginResponse> {
    ...
    async loginWithAuth0(username: string, password: string): Promise<LoginResponse> {
        try {
