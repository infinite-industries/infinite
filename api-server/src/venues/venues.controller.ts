...
export class VenuesController {
  ...
  getAll(
    @Query('includeDeleted') includeDeleted: IncludeDeletedFlag = 'no',
  ...
  getGpsFromAddress(
    @Body() req: GetGPSCoordinatesFromAddressRequest,
  ): Promise<GetGPSCoordinatesResponse> {
    const { street, city, zip, state } = req;

    return this.gpsService
      .getCoordinatesFromAddress({ street, city, state, zip })
      .then(
        (gpsCoordinates) =>
          new GetGPSCoordinatesResponse({
            gpsCoordinates,
          }),
      );
  }
