...
describe('CurrentEvents (e2e)', () => {
  ...
  it('sorts multi-date events by most recent non-expired start-time', async () => {
    ...
    const singleDayEventTimes1 = [getDateTimePair(getTimePlusX(today, 3))];

    const singleDayEventTimes2 = [getDateTimePair(getTimePlusX(today, 7))];
  ...
  it('filters expired events from multi-day events and finds correct first day/last day times', async () => {
    ...
    return server
      ...
      .then(async (response) => {
        ...
        expect(new Date(remainingTimes[0].start_time)).toEqual(
          firstDayTime.start_time,
        );
  ...
  it('returns events with all expected field values', async () => {
    ...
    return server
      ...
      .then(async (response) => {
        ...
        expect(event.date_times[0].start_time).toEqual(
          futureTime.start_time.toISOString(),
        );
