    ...
    methods: {
      adjustCardSpacing: function () {
        ...
        if (window.innerWidth > 480) {
          const card_width = VERTICAL_CARD_SIZE
          const card_total = this.calendar_events.length
          const list_viewer_width = this.$refs.list.clientWidth

          // number of cards that fit in one row is the width of the list divided by the width of a card
          const row_length = Math.floor(list_viewer_width / card_width)
          const shim_equiv = card_total % row_length
          ...
          let shim_width = '0'
          if (shim_equiv > 0) shim_width = (row_length - shim_equiv) * card_width
