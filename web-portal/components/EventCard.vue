  ...
  export default {
    ...
    computed: {
      ...
      when_date: function () {
        ...
        return firstDay ? momenttz(firstDay.start_time).tz(firstDay.timezone || this.$config.public.timezoneDefault).format('dddd, MMMM Do') : null
      },
      when_time: function () {
        const calendar = this.calendar_event
        const dateTimes = calendar.date_times
        const firstDay = dateTimes[0]
        const output_string = firstDay ? momenttz(firstDay.start_time).tz(firstDay.timezone || this.$config.public.timezoneDefault).format('h:mma - ') + momenttz(firstDay.end_time).tz(firstDay.timezone || this.$config.public.timezoneDefaul).format('h:mma z') : null
